...
{
    if(activeEditor())
        activeEditor()->textCursor().deletePreviousChar();
}
...
void MainWindow::autoLinefeed()
{
    if(activeEditor())
        activeEditor()->autoLinefeed();
}
...
void MainWindow::setFont()
{
    if(activeEditor())
    {
        ...
        bool ok;
        QFont font=fontdialog.getFont(&ok,this->font());
        if(ok)
...
void MainWindow::updateActions()
{
    ...
    delAction->setEnabled(hasEditor);
    findAction->setEnabled(hasEditor);
    findNextAction->setEnabled(hasEditor);
    findNextAction->setEnabled(hasEditor);
    selectAllAction->setEnabled(hasEditor);
    dateAction->setEnabled(hasEditor);
    replaceAction->setEnabled(hasEditor);
    jumpAction->setEnabled(hasEditor);
    autoLinefeedAction->setEnabled(hasEditor);
...
//    void setFont();
    ...
    jumpAction=new QAction(tr("&Goto"),this);
    jumpAction->setShortcut(tr("Ctrl+G"));
    connect(jumpAction,SIGNAL(triggered()),this,SLOT(jump()));

    selectAllAction=new QAction(tr("Select&All"),this);
    connect(selectAllAction,SIGNAL(triggered()),this,SLOT(selectAll()));

    dateAction=new QAction(tr("&Date"),this);
    dateAction->setShortcut(tr("F5"));
    connect(dateAction,SIGNAL(triggered()),this,SLOT(date()));

    autoLinefeedAction=new QAction(tr("&AutoLinefeed"),this);
    connect(autoLinefeedAction,SIGNAL(triggered()),this,SLOT(autoLinefeed()));
...
void MainWindow::createMenus()
{
    ...
    formatMenu->addAction(autoLinefeedAction);
